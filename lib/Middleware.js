...
module.exports = class Middleware extends Strategy {
  ...
  async authenticate(req, options = {}) {
    ...
    let keyDoc;
    try {
      keyDoc = await getKey({req, keyId: parsed.keyId, options});
    } catch(err) {
    ...
    try {
      await validateRequest({req, keyDoc, options});
    } catch(err) {
      return self.error(err);
    }
    ...
    let user;
    try {
      user = await getUser({req, keyDoc, options});
    } catch(err) {
      return self.error(err);
    }
  ...
  async ldGetKey({keyId, options = {}}) {
    ...
    if(self.dereferenceUrlScheme[scheme]) {
      if(scheme === 'did') {
        return self.getDidPublicKey({keyId, options});
      }
      if(scheme === 'https') {
        return self.getHttpsPublicKey({keyId, options});
      }
    }
  ...
  async getDidPublicKey({keyId, options = {}}) {
    // TODO: utilized did-io API
    return didio.get(keyId);
    // TODO: check key...
    // // pass `options.proof` through
    // const checkOptions = {};
    // if(options.proof) {
  ...
  async getHttpsPublicKey({keyId, options = {}}) {
    const self = this;
    let keyDoc;
    try {
      keyDoc = await axios.get(keyId).then(res => res.data);
    } catch(err) {
      throw new HttpSignatureError(
