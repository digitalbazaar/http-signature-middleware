...
module.exports = class Middleware extends Strategy {
  ...
  async _parseObjectCapabilities({req, parsed, options}) {
    ...
    if(ocapValue) {
      ...
      for(const ocap of ocaps) {
        ...
        // decode ocap value (it must be base64url encoded)
        try {
          ocap.value = base64url.decode(ocap.value);
        } catch(e) {
          throw new HttpSignatureError(
        ...
        // decode ocap action (it must be base64url encoded)
        if(ocap.action) {
          try {
            ocap.action = base64url.decode(ocap.action);
          } catch(e) {
            throw new HttpSignatureError(
              '"Object-Capability" header "action" encoding error.',
              'SyntaxError');
          }
        }
        ...
        if(ocap.type === 'url') {
          ...
          // resolve ocap
          resolved.push(getOcap({id: ocap.value, parsed, options}));
        } else if(ocap.type === 'ocapld') {
          try {
            const x = JSON.parse(ocap.value);
  ...
  // Note: capabilities may have their own invocations attached ... these
  //   should be given precendence over the http signature, this allows for
  //   different keys to be used to invoke different ocaps should the use
  //   case arise
};
