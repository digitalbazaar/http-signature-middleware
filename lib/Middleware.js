...
module.exports = class Middleware extends Strategy {
  ...
  async authenticate(req, options = {}) {
    ...
    if(req.capabilities) {
      const validateObjectCapabilities = this.use('validateObjectCapabilities');
      if(!validateObjectCapabilities) {
        return this.error(new HttpSignatureError(
          '"Object-Capability" header is not supported.', 'NotSupportedError'));
      }
      try {
        await validateObjectCapabilities({req, keyDoc, parsed, options});
      } catch(err) {
        return this.error(err);
      }
    }
  ...
  async _parseObjectCapabilities({req, parsed, options}) {
    const ocap = req.header('object-capability');
    if(ocap) {
      ...
      // parse ocaps (base64url)
      const ocaps = ocap
        .split(',')
        .map(x => base64url.decode(x.trim()).trim());

      const getObjectCapability = this.use('getObjectCapability') ||
        _getObjectCapability;

      // resolve all ocaps
      const resolved = [];
      for(const ocap of ocaps) {
        // ocaps can be URLs or a stringified JSON object
        if(ocap.startsWith('{')) {
          try {
            resolved.push(JSON.parse(ocap));
          } catch(e) {
            throw new HttpSignatureError(
              '"Object-Capability" decoded value must be a JSON object ' +
              'or a URL.', 'SyntaxError');
          }
        } else {
          // resolve ocap
          getObjectCapability({id: ocap, parsed, options});
        }
      }

      req.capabilities = await Promise.all(resolved);
    }
  }

  // TODO: add validateObjectCapabilitiesLd
};

function _getObjectCapability() {
